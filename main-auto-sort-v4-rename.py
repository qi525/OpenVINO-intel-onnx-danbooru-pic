...
def main():
            ...
            try:
                ...
                with tqdm(total=len(successful_original_paths), desc=f"[目录{dir_idx}] 后处理", leave=False, unit="张") as pbar:
                    for j in range(output_tensor.shape[0]):
                        ...
                        # 执行文件重命名
                        rename_success, rename_result, new_file_path = rename_image_with_tags(original_image_path, tags_list, classifier)
                        if rename_success:
                            result_entry["重命名后的文件名"] = rename_result
                            organize_stats['organized'] += 1
                        elif rename_result == "无有效标签" or rename_result == "文件名无需更改":
    ...
    print("\n--- 整体文件重命名统计 ---")
    print(f"成功重命名: {all_organize_stats['organized']}")
    print(f"跳过重命名: {all_organize_stats['skipped_organize']}")
    print(f"重命名失败: {all_organize_stats['failed_organize']}")
