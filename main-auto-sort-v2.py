...
def organize_images_immediately(image_path, predicted_label):
    ...
    try:
        ...
        logging.info(f"成功组织：{filename} -> {predicted_label}/ 目标路径: {dst_path}")
...
def main():
    ...
    while len(completed_images) < total_images_in_folder:
        ...
        try:
            ...
            for j in range(output_tensor.shape[0]):
                ...
                # 尝试组织文件并记录目标路径
                if auto_organize:
                    organize_success, target_path, organize_error = organize_images_immediately(original_image_path, predicted_label)
                    if organize_success:
    ...
    for original_img_path in image_files:
        if original_img_path not in completed_images:
            logging.critical(f"WARNING: 图片 {original_img_path} 未在处理循环中被标记完成。")
            results_data.append({
